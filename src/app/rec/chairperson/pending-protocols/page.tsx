...
export default function PendingProtocolsPage() {
  const router = useRouter();
  const [openDropdownId, setOpenDropdownId] = useState<string | null>(null);
  const [searchValue, setSearchValue] = useState("");

  // ⚡ Real-time data for pending protocols only (not accepted)
  const { protocols: pendingProtocols, loading, error } = useRealtimeProtocols({
    collectionName: SUBMISSIONS_COLLECTION,
    statusFilter: 'pending',
    enabled: true,
  });

  // Sort protocols by creation date (most recent first) - Using type system
  ...
  const handleAssignReviewer = (protocolId: string) => {
    // Navigate to protocol detail page where reviewer assignment can be done
    router.push(`/rec/chairperson/protocol/${protocolId}`);
  };

  const handleSendMessage = (protocolId: string) => {
    // Navigate to protocol detail page where messaging can be done
    router.push(`/rec/chairperson/protocol/${protocolId}`);
  };
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center">
        <p className="text-destructive mb-4">Error: {error.message || 'Failed to load pending protocols'}</p>
    ...
    <div className="space-y-6 p-4 md:p-6 animate-in fade-in duration-500">
      <Card className="border-[#036635]/10 dark:border-[#FECC07]/20 transition-all duration-300 hover:shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150 overflow-hidden p-0">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            placeholder="Search pending protocols..."
            value={searchValue}
        ...
        <CardContent className="p-6 pb-6">
          <DataTable
            columns={columns}
            data={protocols}
            searchPlaceholder="Search pending protocols..."
            showSearch={false}
            showPagination={true}
            pageSize={10}
