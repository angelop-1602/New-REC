...
export class UnifiedProtocolService {
  ...
  static async getProtocol(protocolId: string, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<Protocol | null> {
    try {
      ...
      throw new Error(`Failed to fetch protocol: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  static async createProtocol(protocolInput: ProtocolInput, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<Protocol> {
    try {
      ...
      // Validate protocol (with temporary id for validation)
      const validation = validateProtocol({ ...protocolData, id: 'temp' } as Protocol);
      if (!validation.isValid) {
        throw new Error(`Protocol validation failed: ${validation.errors.join(', ')}`);
      }
      
      // Convert to old format for storage
      const oldFormat = transformFromUnifiedProtocol({ ...protocolData, id: 'temp' } as Protocol);
      
      // Save to Firebase
      const docRef = doc(collection(db, COLLECTIONS[collectionName]));
      await setDoc(docRef, oldFormat);
      
      return { ...protocolData, id: docRef.id } as Protocol;
    } catch (error) {
      console.error('Error creating protocol:', error);
      throw new Error(`Failed to create protocol: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  static async updateProtocol(protocolId: string, updates: Partial<Protocol>, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<Protocol> {
    try {
      ...
      throw new Error(`Failed to update protocol: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * Delete protocol
   */
  static async deleteProtocol(protocolId: string, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<void> {
    try {
      const docRef = doc(db, COLLECTIONS[collectionName], protocolId);
      await deleteDoc(docRef);
    } catch (error) {
      console.error('Error deleting protocol:', error);
      throw new Error(`Failed to delete protocol: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async moveProtocol(protocolId: string, fromCollection: keyof typeof COLLECTIONS, toCollection: keyof typeof COLLECTIONS): Promise<void> {
    try {
      ...
      throw new Error(`Failed to move protocol: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export class UnifiedAssessmentService {
  ...
  static async getAssessment(protocolId: string, formType: string): Promise<Assessment | null> {
    try {
      const docRef = doc(db, COLLECTIONS.PROTOCOLS, protocolId, COLLECTIONS.ASSESSMENTS, formType);
      const docSnap = await getDoc(docRef);
      ...
      throw new Error(`Failed to fetch assessment: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async getProtocolAssessments(protocolId: string): Promise<Assessment[]> {
    try {
      const querySnapshot = await getDocs(collection(db, COLLECTIONS.PROTOCOLS, protocolId, COLLECTIONS.ASSESSMENTS));
      ...
      throw new Error(`Failed to fetch assessments: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async saveAssessment(assessmentInput: AssessmentInput, protocolId: string, formType: string): Promise<Assessment> {
    try {
      ...
      throw new Error(`Failed to save assessment: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async deleteAssessment(protocolId: string, formType: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to delete assessment: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export class UnifiedReviewerService {
  ...
  static async getReviewer(reviewerId: string): Promise<Reviewer | null> {
    try {
      ...
      throw new Error(`Failed to fetch reviewer: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async getAllReviewers(): Promise<Reviewer[]> {
    try {
      ...
      throw new Error(`Failed to fetch reviewers: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async getActiveReviewers(): Promise<Reviewer[]> {
    try {
      ...
      throw new Error(`Failed to fetch active reviewers: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async createReviewer(reviewerInput: ReviewerInput): Promise<Reviewer> {
    try {
      const now = Timestamp.now();
      const reviewerData: Omit<Reviewer, 'id'> = {
        ...reviewerInput,
        createdAt: now,
        updatedAt: now,
      };
      
      // Validate reviewer (with temporary id for validation)
      const validation = validateReviewer({ ...reviewerData, id: 'temp' } as Reviewer);
      if (!validation.isValid) {
      ...
      await setDoc(docRef, reviewerData);
      
      return { ...reviewerData, id: docRef.id } as Reviewer;
    } catch (error) {
      console.error('Error creating reviewer:', error);
      throw new Error(`Failed to create reviewer: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async updateReviewer(reviewerId: string, updates: Partial<Reviewer>): Promise<Reviewer> {
    try {
      ...
      throw new Error(`Failed to update reviewer: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async deleteReviewer(reviewerId: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to delete reviewer: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export class UnifiedDecisionService {
   ...
   */
  static async getDecision(protocolId: string, collectionName: keyof typeof COLLECTIONS | string = 'PROTOCOLS'): Promise<Decision | null> {
    try {
      // Validate protocolId
   ...
   */
  static async saveDecision(decisionInput: DecisionInput, protocolId: string, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<Decision> {
    try {
      const now = Timestamp.now();
      ...
      throw new Error(`Failed to save decision: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * Delete decision
   */
  static async deleteDecision(protocolId: string, collectionName: keyof typeof COLLECTIONS = 'PROTOCOLS'): Promise<void> {
    try {
      const docRef = doc(db, COLLECTIONS[collectionName], protocolId, COLLECTIONS.DECISIONS, 'details');
      await deleteDoc(docRef);
    } catch (error) {
      console.error('Error deleting decision:', error);
      throw new Error(`Failed to delete decision: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
