 ...
 */
import { toDate, toLocaleDateString, FirestoreDate } from '@/types';

function formatFirestoreDate(timestamp: unknown): string {
  try {
    if (!timestamp) return '';
    
    const date = toDate(timestamp as FirestoreDate);
    if (!date) return '';
