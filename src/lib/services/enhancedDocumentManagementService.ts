...
export class EnhancedDocumentManagementService {
  ...
  static async createDocument(
    ...
    try {
      ...
      if (existingDocumentId) {
        ...
        console.log(`Document request fulfilled with ID: ${existingDocumentId}`);
        ...
        const enhancedDocument: Omit<EnhancedDocument, 'id'> = {
        ...
        const docRef = await addDoc(documentsRef, enhancedDocument);
  ...
  static async addDocumentVersion(
    ...
    try {
      ...
      const updateData = {
        ...
        currentStatus: 'pending',
        updatedAt: now,
        versions: [...currentData.versions, newVersion]
      };
      
      await updateDoc(documentRef, updateData);
      
      console.log(`New version ${newVersionNumber} added to document ${documentId}`);
      return true;
    } catch (error) {
  ...
  /**
   * Cancel a document request (set status to cancelled)
   */
  static async cancelDocumentRequest(
    protocolId: string,
    documentId: string
    ...
    try {
      ...
      console.log(`Document request ${documentId} cancelled`);
      return true;
    } catch (error) {
      console.error('Error cancelling document request:', error);
      throw error;
