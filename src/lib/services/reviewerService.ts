...
class ReviewerService {
  ...
  async assignReviewers(
    ...
    try {
      ...
      if (researchType === 'EX' && exemptionSubType && requirements.hasSubTypes && requirements.subTypes) {
  ...
  async checkOverdueReviewers(protocolId: string): Promise<void> {
    try {
      ...
      for (const docSnap of snapshot.docs) {
        ...
        if (deadline < now && assignment.reviewStatus === 'pending') {
  ...
  async reassignReviewer(
    ...
    try {
      ...
      const now = new Date();
      const originalDeadline = oldAssignment.deadline?.toDate?.() || new Date(oldAssignment.deadline);
  ...
  async getReassignedProtocols(reviewerId: string): Promise<any[]> {
    try {
      ...
      return reassignedProtocols.sort((a, b) => {
        const dateA = a.reassignedAt?.toDate?.() || new Date(a.reassignedAt);
        const dateB = b.reassignedAt?.toDate?.() || new Date(b.reassignedAt);
