 ...
 * Note: Includes title initials (Dr., Ms., Prof., etc.) in the code
 */
export class ReviewerCodeGenerator {
  /**
   * Extract initials from a full name
   * @param name Full name (e.g., "Dr. Janette Fermin")
   * @returns Initials (e.g., "DRJF") - includes all letters from titles
   */
  private static extractInitials(name: string): string {
    // Keep titles and extract all letters from titles, first letter from names
    ...
    const initials = words
      ...
      .map(word => {
        ...
        // Check if it's a common title (Dr, Ms, Mrs, Mr, Prof, Professor, etc.)
        const titlePattern = /^(DR|MS|MRS|MR|PROF|PROFESSOR)$/i;
        if (titlePattern.test(cleaned)) {
          // For titles, use all letters (DR -> DR, MS -> MS, PROF -> PROF)
          // Handle "PROFESSOR" as "PROF" to keep codes shorter
          if (cleaned === 'PROFESSOR') {
            return 'PROF';
          }
          return cleaned;
        } else {
          // For regular names, use only first letter
  ...
  static validateCode(code: string): boolean {
    // Allow 2-8 uppercase letters (to accommodate titles like DR, MS, PROF, etc. + name initials)
    const pattern = /^[A-Z]{2,8}-\d{3}$/;
  ...
  static parseCode(code: string): { initials: string; number: number } | null {
    // Allow 2-8 uppercase letters (to accommodate titles like DR, MS, PROF, etc.)
    const match = code.match(/^([A-Z]{2,8})-(\d{3})$/);
    if (match) {
      return {
        initials: match[1],
